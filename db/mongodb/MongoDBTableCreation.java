...
public class MongoDBTableCreation {
  ...
  public static void main(String[] args) throws ParseException {
		...
		// Step 3, create new collections with UNIQUE constraints
		IndexOptions options = new IndexOptions().unique(true);
		db.getCollection("users").createIndex(new Document("user_id", 1), options);//1 is ascending, -1 is descending
		db.getCollection("items").createIndex(new Document("item_id", 1), options);//new Document is to create an option as {} in mongodb statement
