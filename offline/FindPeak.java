...
public class FindPeak{
    ...
    // Use LocalTime.isAfter/isBefore to compare to objects with BS
	...
	public static void main(String [] args) {
		...
		// Construct mapper function in js(mongodb's requirements)
		// function() {
		// 	if (this.url.startswith("/Titan")) {  //only study the requests on /titan
		// 		emit(this.time.substring(0, 5), 1);  //hour:min as key
		// 	}
		// }
		...
		results.forEach(new Block<Document>() {
			//what we get from mapreduce is total num at a xx:xx:xx, but we need to count with a coarse scale 
			@Override
